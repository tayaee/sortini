 ...
 * 9. Output always ends with a visible blank line (double newline).
 *
 * @param {string} content - Raw content of INI file
 * @returns {string} - Sorted INI content
 */
function sortIniContent(content, newlineFormat = 'windows') {
  let format = typeof newlineFormat === 'object' && newlineFormat !== null
  ...
  // Always end with a visible blank line (double newline) so that it contains an empty line when viewed
